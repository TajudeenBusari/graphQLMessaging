...
public class RabbitMqPublisher : IMessagePublisher, IAsyncDisposable
{
        ...
        CancellationToken cancellationToken = default) where TMessage : class
    {
        ...
        _logger.LogInformation(
            "Publishing message {MessageType} to exchange {Exchange} with routing key {RoutingKey}",
            typeof(TMessage).FullName,
            destination.ExchangeOrTopic,
            destination.RoutingKeyOrSubject);
        await _channel.BasicPublishAsync(
            destination.ExchangeOrTopic,
            destination.RoutingKeyOrSubject, //or derived from routing strategy
            body,
            cancellationToken);
    }
